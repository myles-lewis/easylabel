...
We recommend installing the development version of plotly which has better
resolution of scatter plots.
...
using `colScheme = 'blue'` or can be set to change with level of a factor 
variable within `data` by setting `col`. Currently colour gradients are not 
supported.

## Shape

Marker shapes can either be set as a single shape using `shapeScheme = 21`, 
based on the base graphics `pch` symbol scheme (see `points()`). Shapes and 
colours can be combined.

## Size

The size of all markers can be adjusted by setting `size` to a single number 
e.g. `size = 1`. Size can be assigned to a column of continuous data within
`data` to produce a bubble chart. The size of points is automatically scaled to 
fit within a range of point sizes, determined by `scaleRange`.
...
easyMAplot(volc2, fdrcutoff = c(0.05, 0.01, 0.001), size = 0.8, useQ = TRUE,
       alpha = 0.75, outline_col = NA,
       colScheme = colScheme)
```
...
easyMAplot(volc2, fdrcut = c(0.05, 0.01, 0.001), size = 0.8, useQ = TRUE,
       alpha = 0.75, outline_col = NA,
       fullGeneNames = TRUE,
       colScheme = c('darkgrey', brewer.pal(9, 'RdYlBu')[c(7:9, 3:1)]),
       rectangles = TRUE,
