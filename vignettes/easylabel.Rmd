...
`line_col` to single colours, label text and label lines can also be set to 
mirror the colour of each point by setting `text_col = "match"` and 
`line_col = "match"` respectively. Rectangle fill colour or rectangle outline 
colour can also be set to match each point's colour by setting 
`rect_col = "match"` or `border_col = "match"` (not currently supported in 
plotly).
...
easylabel(gapminder[gapminder$year == 2007, ], x = 'gdpPercap', y = 'lifeExp',
          ...
          rect_col = "match", border_radius = 0, padding = 5,
          zeroline = FALSE, showgrid = "y")
```
