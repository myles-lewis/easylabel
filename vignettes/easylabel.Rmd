...
`"x"` or `"y"` then gridlines are shown only for that axis.
...
plotly, to prevent plotly becoming sluggish or unusable, which happens anywhere 
between 200,000 to 400,000 or more points. To plot large datasets such as 
...
`plotly_filter` specifies the name of this column. Saving to pdf retains the 
full dataset and all points will be plotted. This argument is used by the 
`easyManhattan()` function (see [Manhattan plot]).
...
* We recommend installing at least version 4.10.0 of plotly/R which has much 
  better on-screen resolution of scatter plots for a better user experience. 
  Note that exporting to pdf is unaffected by this.
